...
- Широкополостный пинг:

  чтоб отвечали все машины из бродкаст домена, нужно на машине, с которой пингуется сетка, отключить параметр в ядре:

      echo 0 >  /proc/sys/net/ipv4/icmp_echo_ignore_broadcasts

  далее стандартно, команда broadcast  пинга:

      ping -b 192.168.1.**255** - это броадкаст адресс для сети 192.168.1.1/24
